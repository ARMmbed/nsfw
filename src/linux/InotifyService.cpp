...
InotifyService::InotifyService(std::shared_ptr<EventQueue> queue, std::string path, bool followSymlinks):
  mEventLoop(NULL),
  mQueue(queue),
  mTree(NULL) {
  ...
  mTree = new InotifyTree(mInotifyInstance, path, followSymlinks);
